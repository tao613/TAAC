...
def get_args():
    ...
    # 训练参数（需要与训练时保持一致）
    parser.add_argument('--batch_size', default=256, type=int, help='推理批次大小')
    ...
    parser.add_argument('--hidden_units', default=64, type=int, help='隐藏层维度')
    parser.add_argument('--num_blocks', default=2, type=int, help='Transformer块数量')
    parser.add_argument('--num_epochs', default=4, type=int, help='训练轮数（仅为兼容性）')
    ...
    parser.add_argument('--l2_emb', default=1e-6, type=float, help='Embedding L2正则化权重')
    ...
    parser.add_argument('--use_inbatch_negatives', action='store_true', default=True,
                       help='是否使用In-batch Negatives策略（默认启用，与训练保持一致）')
    parser.add_argument('--use_infonce_loss', action='store_true', default=True,
                       help='是否使用InfoNCE Loss（默认启用，与训练保持一致）')
    parser.add_argument('--temperature', default=0.07, type=float,
                       help='InfoNCE Loss的温度参数（与训练保持一致）')
