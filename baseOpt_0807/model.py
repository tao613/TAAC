...
class BaselineModel(torch.nn.Module):
    ...
    def forward_inbatch_negatives(
        self, user_item, pos_seqs, mask, next_mask, seq_feature, pos_feature, max_negatives_per_query=100
    ):
        """
        使用In-batch Negatives的前向传播方法
        ...
        if valid_count <= 1:
            ...
            # 内存优化：限制每个查询的负样本数量
            # 计算实际的负样本数量（考虑内存限制）
            available_negatives = valid_count - 1
            actual_neg_count = min(available_negatives, max_negatives_per_query)
            
            # 重新组织为 [valid_count, actual_neg_count, hidden_units]
            neg_embs = torch.zeros(valid_count, actual_neg_count, query_embs.size(-1), device=self.dev)
            
            for i in range(valid_count):
                # 获取第i个查询的负样本（排除第i个位置）
                available_indices = torch.cat([torch.arange(i, device=self.dev), 
                                             torch.arange(i+1, valid_count, device=self.dev)])
                
                if available_indices.size(0) <= actual_neg_count:
                    # 如果可用负样本不够，全部使用
                    neg_embs[i, :available_indices.size(0)] = pos_embs_flat[available_indices]
                else:
                    # 随机采样指定数量的负样本
                    selected_indices = available_indices[torch.randperm(available_indices.size(0), device=self.dev)[:actual_neg_count]]
                    neg_embs[i] = pos_embs_flat[selected_indices]
