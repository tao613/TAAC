#!/usr/bin/env python3
"""
简化版推理脚本 - 跳过semantic_id处理
专门解决semantic_id_dict属性错误
"""
...
def smart_load_model(model, checkpoint_path, device):
    ...
    if adaptive_keys:
        print(f"检测到自适应相似度权重: {len(adaptive_keys)} 个")
    ...
    if missing_keys:
        print(f"模型中缺失的权重: {len(missing_keys)} 个")
    if unexpected_keys:
        print(f"checkpoint中多余的权重: {len(unexpected_keys)} 个")
...
def get_candidate_emb(indexer, feat_types, feat_default_value, mm_emb_dict, model):
    """生成候选物品embedding - 简化版，不使用semantic_id"""
    ...
    for item_reid, item_id in tqdm(indexer.items(), desc="Processing items"):
        ...
        # 跳过semantic_id特征，使用默认值
        for feat_type in feat_types.get('semantic_array', []):
            feat[feat_type] = feat_default_value[feat_type]
...
def infer():
    ...
    # 确保semantic_id_dict存在，即使为空
    if not hasattr(test_dataset, 'semantic_id_dict'):
        test_dataset.semantic_id_dict = {}
        print("手动设置了空的semantic_id_dict")
    else:
        print(f"semantic_id_dict 状态: {type(test_dataset.semantic_id_dict)}")
